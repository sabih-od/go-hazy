...
//        $productt = Product::where('slug','=',$slug)->firstOrFail();
        $productt = Product::with('productVariants')->where('slug', '=', $slug)->firstOrFail();

        $variation = !empty($productt->productVariants) ? $productt->productVariants->groupBy('option_type') : [];
        ...
        return view('frontend.product-detail', compact('productt', 'curr', 'vendors', 'affilate_user', 'variation'));
    ...
    public function getProductVariation(Request $request)
    {
        ...
        $option_ids = [];
        $data['items'] = ProductVariation::query()
            ->whereIn('option_id', $request->ids)
