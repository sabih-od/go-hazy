...
class CatalogController extends FrontBaseController
{
    ...
    public function subCategory(Request $request, $slug = null, $slug1 = null, $slug2 = null)
    {
        if ($request->view_check) {
            session::put('view', $request->view_check);
        }
        ...
        if (!empty($slug1)) {
            $subcat = Subcategory::where('slug', $slug1)->get();
            $data['subcat'] = $subcat;
//            dd($subcat);
        }
        ...
        $data['prods'] = $prods;
        $sub = Product::where('subcategory_id',$subcat);
//        dd($sub);
        if ($request->ajax()) {
            $data['ajax_check'] = 1;
            return view('frontend.ajax.category', $data, $sub);
        }
        return view('frontend.subcat')->with('sub',$sub);
    }
