                ...
                <div class="col-xl-8">
                    <div class="user-profile-details">
                        <div class="row">
                            @foreach($subs as $sub)
                                <div class="col-lg-6">
                                    <div class="elegant-pricing-tables">
                                        ...
                                        @if(!empty($package))
                                            @if($package->subscription_id == $sub->id)
                                                <a href="javascript:;"
                                                   class="themeBtn">{{ __('Current Plan') }}</a>
                                                <br>
                                                @if(Carbon\Carbon::now()->format('Y-m-d') > $user->date)
                                                    <small
                                                        class="hover-white">{{ __('Expired on:') }} {{ date('d/m/Y',strtotime($user->date)) }}</small>
                                                @else
                                                    <small
                                                        class="hover-white">{{ __('Ends on:') }} {{ date('d/m/Y',strtotime($user->date)) }}</small>
                                                @endif
                                                <a href="{{route('user-vendor-request',$sub->id)}}" class="hover-white"><u>{{ __('Renew') }}</u></a>
                                            @else
                                                <a href="{{route('user-vendor-request',$sub->id)}}"
                                                   class="themeBtn">{{ __('Get Started') }}</a>
                                            @endif
                                        @else
                                            <a href="{{route('user-vendor-request',$sub->id)}}"
                                               class="themeBtn">{{ __('Get Started') }}</a>
